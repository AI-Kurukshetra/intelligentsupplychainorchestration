...
import { facilityCreateSchema } from "@/types/schemas";
...
export async function POST(request: NextRequest) {
  ...
  const parsed = facilityCreateSchema.safeParse(await request.json());
  ...
  if (error) return sendError(error.message, 500);
  return sendSuccess(data, 201, { message: "Facility created." });
}
